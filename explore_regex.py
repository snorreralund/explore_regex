...
class ExploreRegex():
    ...
    def explore_pattern(self,pattern,n_samples=10,context=10,shuffle=True):
        ...
        longest_match_len = len(repr(max(matches, key = len))) #to seperate match and context with sufficient amount of whitespace


        print('Match:{}Context:'.format(' '*(longest_match_len-1)), end = '\n\n') #columnheaders


        for m, c in zip(matches, contexts):


            c = c.replace('\n', '\n{}'.format(' ' * (longest_match_len+5))) #so newlines in context are formatted nicely when printed


            row_divider = '\n{}\n'.format('- '*(longest_match_len + context)) #divider between rows for print()


            match_context_string = repr(m) + ' '*(longest_match_len-len(repr(m))+5) + str(c)


            print(match_context_string, sep = '', end = row_divider)
